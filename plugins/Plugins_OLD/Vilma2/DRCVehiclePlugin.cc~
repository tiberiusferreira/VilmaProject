...
void DRCVehiclePlugin::UpdateStates()
{
  ...
  if (dt > 0)
  {
/*	if(curTime.Double()<60){
		this->handWheelCmd=2;
	}
else if(curTime.Double()<120)
	this->handWheelCmd=-2;
else
	this->handWheelCmd=0;
*/

/*	if(curTime.Double()<180)
{
this->handBrakeCmd=handBrakeLow;
double vel=(flWheelState*flWheelRadius+frWheelState*frWheelRadius)/2.0;
double command=10*(5-vel);
//saturar
if(command>1)
command=1;
else if(command<-1)
...
if(command>0)
{
this->gasPedalCmd=gasPedalHigh*command;
this->brakePedalCmd=brakePedalLow;
}
else if(command<=0)
{
this->brakePedalCmd=brakePedalHigh*command;
this->gasPedalCmd=gasPedalLow;
}
}
else{
this->gasPedalCmd=brakePedalHigh;
this->gasPedalCmd=gasPedalLow;
}*/
...
/*if(curTime.Double()<120){
this->handBrakeCmd=handBrakeLow;
double vel=(3.6/flWheelRadius);
    this->flWheelJoint->SetForce(0, this->frontTorque);
    this->frWheelJoint->SetForce(0, this->frontTorque);
    this->flWheelJoint->SetVelocity(0, vel);
    this->frWheelJoint->SetVelocity(0, vel);
//asi no sirven las ruedas
/*math::Vector3 vel(3.6,0,0);
